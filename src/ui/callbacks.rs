...
use crate::model::{DownloadProgress, DownloadTarget, Language};
...
pub struct UiCallbacks {
    ...
    pub set_feature_progress: Arc<dyn Fn(DownloadTarget, DownloadProgress) + Send + Sync>,
    ...
    pub set_tts_voices: Arc<dyn Fn(Vec<TtsVoiceListItem>) + Send + Sync>,
...
pub fn create_ui_callbacks(app: QPointer<AppBridge>) -> UiCallbacks {
    ...
    let set_feature_progress = queued_callback(move |args: (DownloadTarget, DownloadProgress)| {
        if let Some(app) = progress_app.as_pinned() {
            app.borrow_mut().set_feature_progress_value(&args.0, args.1);
        }
    });
    ...
    let set_tts_voices = queued_callback(move |items: Vec<TtsVoiceListItem>| {
        if let Some(app) = tts_voices_app.as_pinned() {
            app.borrow_mut().set_tts_voices_value(items);
        }
    });
    ...
    UiCallbacks {
        ...
        set_feature_progress: Arc::new(move |target, progress| {
            set_feature_progress((target, progress))
        }),
        ...
        set_tts_voices: Arc::new(set_tts_voices),
